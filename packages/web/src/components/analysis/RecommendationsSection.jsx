    ...
    <div className="space-y-6 animate-fade-in">
      {Object.entries(grouped).map(([category, tips]) => {
        ...
        return (
          <div
            key={category}
            className="bg-void-mid/30 rounded-xl border border-void-bright/10 p-4"
          >
            <div className="flex items-center gap-2 mb-4">
              <i
                className={`fas ${catStyle.icon || 'fa-circle'} ${catStyle.color || 'text-white'}`}
              />
              <h3
                className={`text-sm font-semibold uppercase tracking-wider ${catStyle.color || 'text-white'}`}
              >
      ...
      {secondaryTips.length > 0 && (
        ...
        >
          {showMore
            ? t('analysis.showLess')
            : t('analysis.showMore', { count: secondaryTips.length })}
        </button>
      )}
    </div>
  );
